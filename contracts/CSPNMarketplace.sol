...
/* CSPN NFT Marketplace
    ...
    & support Royalty
*/
contract CSPNNFTMarketplace is Ownable, ReentrancyGuard {
    ICSPNNFTFactory private immutable cspnNFTFactory;
    ...
    constructor(
        ...
        ICSPNNFTFactory _cspnNFTFactory
    ) {
        require(_platformFee <= 10000, "can't more than 10 percent");
        platformFee = _platformFee;
        feeRecipient = _feeRecipient;
        cspnNFTFactory = _cspnNFTFactory;
    }

    modifier isCSPNNFT(address _nft) {
        require(cspnNFTFactory.isCSPNNFT(_nft), "not CSPN NFT");
        _;
    }
    ...
    function listNft(
        ...
        address _payToken,
        uint256 _price
    ) external isCSPNNFT(_nft) isPayableToken(_payToken) {
    ...
    function buyNFT(
        ...
        ICSPNNFT nft = ICSPNNFT(listedNft.nft);
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        ICSPNNFT nft = ICSPNNFT(offer.nft);
        address royaltyRecipient = nft.getRoyaltyRecipient();
        uint256 royaltyFee = nft.getRoyaltyFee();
    ...
    function resultAuction(address _nft, uint256 _tokenId) external {
        ...
        ICSPNNFT cspnNft = ICSPNNFT(_nft);
        address royaltyRecipient = cspnNft.getRoyaltyRecipient();
        uint256 royaltyFee = cspnNft.getRoyaltyFee();
